...
export function resolveTelegramSupportIssueLabel(
  ...
  switch (resolveTelegramSupportIssueCategory(category)) {
    ...
    case 'SERVER':
      return isMyanmar ? 'ဆာဗာ / လမ်းကြောင်း' : 'Server / route issue';
    case 'BILLING':
      return isMyanmar ? 'ငွေတောင်းခံမှု / ကူပွန်' : 'Billing / coupon';
    case 'GENERAL':
    default:
...
export function resolveTelegramSupportIssuePrompt(
  ...
  switch (category) {
    case 'ORDER':
      return isMyanmar
        ? 'ငွေပေးချေမှု၊ screenshot၊ အော်ဒါအခြေအနေ သို့ စစ်ဆေးမှုနောက်ကျမှု ပြဿနာကို ပို့ပါ။ အော်ဒါကုဒ် ရှိပါက ထည့်ပါ။'
        : 'Tell us the payment, screenshot, order-status, or review issue. Include the order code if you have it.';
    case 'KEY':
      return isMyanmar
        ? 'သော့၊ အသုံးပြုမှု၊ ချိတ်ဆက်မှု၊ သက်တမ်းတိုးမှု သို့ မျှဝေစာမျက်နှာ ပြဿနာကို ပို့ပါ။ သော့အမည် ရှိပါက ထည့်ပါ။'
        : 'Tell us the key, usage, connection, renew, or share-page issue. Include the key name if you have it.';
    case 'SERVER':
      return isMyanmar
        ? 'ဆာဗာအမည်၊ လမ်းကြောင်းပြဿနာ၊ ချိတ်ဆက်မှု နှေးခြင်း သို့ ဒေသဆိုင်ရာ ပြဿနာကို ပို့ပါ။'
        : 'Tell us the server name, route issue, slow connection, or region problem.';
    case 'BILLING':
      return isMyanmar
        ? 'ပြေစာ၊ ငွေတောင်းခံမှု၊ လျှော့စျေး၊ ကူပွန် သို့ ငွေပေးချေမှု နောက်ဆက်တွဲ ပြဿနာကို ပို့ပါ။'
...
export function buildTelegramSupportThreadStartMessage(input: {
  ...
  return buildTelegramCommerceMessage({
    title:
      input.locale === 'my'
        ? '🛟 <b>အကူအညီ တောင်းဆိုချက် စတင်ပြီးပါပြီ</b>'
    ...
    cards: [
      buildTelegramCommerceCard(
        ...
        [
          escapeHtml(prompt),
          input.locale === 'my'
            ? 'Screenshot သို့ ဖိုင်တစ်ခုကို တစ်ချက်တည်း ပို့နိုင်ပါသည်။'
            : 'Send one screenshot or file next if it helps.',
        ],
      ),
    ],
    footerLines: [
      input.locale === 'my'
        ? 'စီမံသူ အဖြေကို ဒီ chat မှာပဲ စောင့်ပါ။'
        : 'Wait for the admin reply in this chat.',
    ],
  });
}
...
export function buildTelegramSupportReplyClosedMessage(locale: SupportedLocale) {
  return locale === 'my'
    ? '🧵 ဤစာတွဲကို ပိတ်ထားပါသည်။ အသစ်စရန် /support ကို သုံးပါ။'
...
export function buildTelegramSupportHubMessage(input: {
  ...
  const quickRouteLines = isMyanmar
    ? [
        '🧾 အော်ဒါ • ငွေပေးချေမှု၊ အထောက်အထား၊ စစ်ဆေးမှု',
        '🔑 သော့ • ချိတ်ဆက်မှု၊ သက်တမ်းတိုးမှု၊ အသုံးပြုမှု',
        '🛠 ဆာဗာ • လမ်းကြောင်း၊ ဒေသ၊ ချိတ်ဆက်မှု နှေးခြင်း',
      ]
    : [
        '🧾 Order • payment, proof, review',
        '🔑 Key • connect, renew, usage',
        '🛠 Server • route, region, slow',
      ];
  const stats = [
    isMyanmar ? `${input.openThreadCount} ခု ဖွင့်ထား` : `${input.openThreadCount} open`,
    isMyanmar ? `${input.recentThreadCount} ခု မကြာသေးမီက` : `${input.recentThreadCount} recent`,
    input.premiumRequestCount > 0
      ? isMyanmar
        ? `${input.premiumRequestCount} ခု ပရီမီယမ်`
  ...
  if (input.latestThread) {
    cards.push(
      buildTelegramCommerceCard(
        isMyanmar ? '🧵 <b>ဖွင့်ထားသော စာတွဲ</b>' : '🧵 <b>Open thread</b>',
        [
    ...
    cards.push(
      buildTelegramCommerceCard(
        ...
        [
          isMyanmar
            ? 'ယခုအချိန်တွင် ဖွင့်ထားသော စာတွဲ မရှိသေးပါ။'
  ...
  if (input.latestPremiumRequest) {
    cards.push(
      buildTelegramCommerceCard(
        isMyanmar ? '💎 <b>နောက်ဆုံး ပရီမီယမ် တောင်းဆိုချက်</b>' : '💎 <b>Latest premium request</b>',
        [
  ...
  cards.push(
    buildTelegramCommerceCard(
      isMyanmar ? '🧭 <b>အမြန်ရွေးချယ်ရန်</b>' : '🧭 <b>Fast lane</b>',
      quickRouteLines,
    ),
  );
  ...
  return buildTelegramCommerceMessage({
    ...
    statsLine: stats,
    cards,
    footerLines: input.supportLinkConfigured
      ? [
          isMyanmar
            ? 'အောက်က အမျိုးအစားတစ်ခုကို စတင်ပါ သို့မဟုတ် ဖွင့်ထားသော စာတွဲကို ပြန်ဖွင့်ပါ။'
            : 'Start a category or reopen an open thread below.',
        ]
      : [getTelegramUi(input.locale).noSupportLink],
  });
}
...
export function getTelegramSupportThreadState(input: {
  ...
  if (input.status === 'ESCALATED') {
    return {
      ...
      label: isMyanmar ? 'ထိန်းချုပ်ခန်းသို့ တင်ထားသည်' : 'Escalated to panel',
    };
  }
  ...
  return {
    code: 'admin' as const,
    label: isMyanmar ? 'စီမံသူ အဖြေကို စောင့်နေ' : 'Waiting for admin',
  };
}
...
export function getTelegramSupportThreadSlaLabel(input: {
  ...
  if (input.thread.firstResponseDueAt) {
    return isMyanmar
      ? `ပထမအဖြေ နောက်ဆုံးချိန် ${formatTelegramDateTime(input.thread.firstResponseDueAt, input.locale)}`
...
export function buildTelegramSupportHubKeyboard(input: {
  ...
  rows.push(
    input.showPremiumShortcut
      ? [
          ...
          {
            text: isMyanmar ? '💎 ပရီမီယမ် အကူအညီ' : '💎 Premium help',
  ...
  if (input.supportLink) {
    rows.push([{ text: isMyanmar ? '🛟 စီမံသူကို ဆက်သွယ်ရန်' : '🛟 Contact admin', url: input.supportLink }]);
  }
...
export function buildTelegramSupportThreadKeyboard(input: {
  ...
  if (input.attachmentUrl?.trim()) {
    rows.push([
      {
        text: isMyanmar ? '🖼 တွဲဖိုင် ဖွင့်မည်' : '🖼 Open attachment',
  ...
  if (input.supportLink) {
    rows.push([{ text: isMyanmar ? '🔗 စီမံသူ တိုက်ရိုက်လင့်ခ်' : '🔗 Direct admin link', url: input.supportLink }]);
  }
...
export function buildTelegramSupportStatusSummaryMessage(input: {
  ...
  const threadCards = input.threads.slice(0, 3).map((thread, index) => {
    ...
    const ageLabel = formatTelegramSupportRelativeAge(thread.updatedAt || thread.createdAt, input.locale);
    return buildTelegramCommerceCard(
      ...
      [
        ...
        `${escapeHtml(ageLabel)} • ${escapeHtml(formatTelegramDateTime(thread.updatedAt || thread.createdAt, input.locale))}`,
      ],
    );
  });
  ...
  const premiumCards = (input.premiumRequests || []).slice(0, 2).map((request) => {
    const replyStateLabel = formatTelegramReplyStateLabel({
      ...
      latestReplySenderType: request.latestReplySenderType || null,
    ...
    return buildTelegramCommerceCard(
      ...
      [
        `${escapeHtml(formatTelegramPremiumSupportTypeLabel(request.requestType, ui))} • ${escapeHtml(
  ...
  if (threadCards.length === 0 && premiumCards.length === 0) {
    return input.locale === 'my'
      ? '🧵 <b>သင့် စာတွဲများ</b>\n\nအကူအညီ စာတွဲ မရှိသေးပါ။ /support ဖြင့် စတင်နိုင်ပါသည်။'
      : '🧵 <b>Your threads</b>\n\nNo support threads yet. Use /support to start one.';
  }

  return buildTelegramCommerceMessage({
    title: input.locale === 'my' ? '🧵 <b>သင့် စာတွဲများ</b>' : '🧵 <b>Your threads</b>',
    statsLine: stats,
    cards: [...threadCards, ...premiumCards].slice(0, 4),
    footerLines: [
      input.locale === 'my'
        ? 'အောက်က စာတွဲကို ဖွင့်ပါ သို့မဟုတ် အသစ်စတင်ပါ။'
        : 'Open a thread below or start a new one.',
    ],
  });
}
...
export function buildTelegramSupportStatusSummaryKeyboard(input: {
  ...
  for (const thread of input.threads.slice(0, 3)) {
    rows.push([
      {
        text: `🧵 ${truncateTelegramSupportButtonLabel(thread.threadCode)}`,
  ...
  for (const request of (input.premiumRequests || []).slice(0, 2)) {
    rows.push([
      {
        text: `💎 ${truncateTelegramSupportButtonLabel(request.requestCode)}`,
  ...
  rows.push([
    ...
    ...(input.premiumRequests && input.premiumRequests.length > 0
      ? [
          {
...
export function buildTelegramSupportThreadStatusMessage(input: {
  ...
  const snapshotLines = [
    ...
    [
      input.thread.relatedOrderCode
        ? `${input.locale === 'my' ? 'အော်ဒါ' : 'Order'}: <b>${escapeHtml(input.thread.relatedOrderCode)}</b>`
        : null,
      input.thread.relatedKeyName
        ? `${input.locale === 'my' ? 'သော့' : 'Key'}: <b>${escapeHtml(input.thread.relatedKeyName)}</b>`
        : null,
    ]
      .filter(Boolean)
      .join(' • ') || null,
  ...
  const nextStepLine =
    state.code === 'user'
      ? input.locale === 'my'
        ? '✍️ လိုအပ်သည့်အချက်ကို အကြောင်းပြန်ပေးပါ။'
        : '✍️ Reply with the missing detail.'
      : state.code === 'escalated'
        ? input.locale === 'my'
          ? '✍️ ဒီ chat မှာပဲ အသေးစိတ် ထပ်ပို့နိုင်ပါသည်။ Panel review ကို ဆက်လုပ်နေပါသည်။'
          : '✍️ Add more detail here if anything changes. Deeper review continues in the panel.'
      : state.code === 'handled'
        ? input.locale === 'my'
          ? '✍️ အကြောင်းပြန်ခလုတ်ကို နှိပ်ပြီး စာတွဲကို ပြန်ဖွင့်နိုင်ပါသည်။'
          : '✍️ Tap Reply to reopen this thread.'
        : input.locale === 'my'
          ? '⏳ စီမံသူ အဖြေကို ဒီ chat မှာ စောင့်ပါ။'
          : '⏳ Wait for the admin reply in this chat.';

  const cards = [
    buildTelegramCommerceCard(
      input.locale === 'my' ? '🧵 <b>စာတွဲ အနှစ်ချုပ်</b>' : '🧵 <b>Thread snapshot</b>',
      snapshotLines,
    ),
    buildTelegramCommerceCard(
      input.locale === 'my' ? '➡️ <b>နောက်တစ်ဆင့်</b>' : '➡️ <b>Next step</b>',
      [escapeHtml(nextStepLine.replace(/^✍️ |^⏳ /, ''))],
    ),
  ];
  ...
  if (latestReply) {
    ...
    const latestReplyLines = [
      `${latestReply.senderType === 'ADMIN' ? (input.locale === 'my' ? 'စီမံသူ' : 'Admin') : input.locale === 'my' ? 'သင်' : 'You'} • ${escapeHtml(
        ...
        ? `${input.locale === 'my' ? 'တွဲဖိုင်' : 'Attachment'}: <b>${escapeHtml(
            latestReply.mediaKind === 'IMAGE'
              ...
              : latestReply.mediaKind === 'FILE'
                ...
                : latestReply.mediaKind,
          )}</b>${latestReply.mediaUrl ? ` • ${input.locale === 'my' ? 'အောက်ပါ ခလုတ်' : 'button below'}` : ''}`
        : null,
      latestReplyPreview
    ...
    cards.push(
      buildTelegramCommerceCard(
        input.locale === 'my' ? '💬 <b>နောက်ဆုံး အကြောင်းပြန်ချက်</b>' : '💬 <b>Last reply</b>',
        latestReplyLines,
      ),
    );
  }
...
export function buildTelegramSupportTriageMessage(input: { locale: SupportedLocale }) {
  ...
  return buildTelegramCommerceMessage({
    title: isMyanmar ? '🛟 <b>အကူအညီ စင်တာ</b>' : '🛟 <b>Support center</b>',
    statsLine: isMyanmar ? 'ငွေပေးချေမှု၊ သော့ နှင့် ဆာဗာ အကူအညီ' : 'Fast help for payments, keys, and server issues',
    cards: [
      buildTelegramCommerceCard(
        isMyanmar ? '🧭 <b>စတင်ရန်</b>' : '🧭 <b>Start here</b>',
        [
          isMyanmar
            ? 'အောက်က အမျိုးအစားကို ရွေးပြီး စာတွဲတစ်ခု စတင်ပါ။'
            : 'Pick a category below to start a thread.',
          isMyanmar
            ? 'လိုအပ်ပါက screenshot သို့ file တစ်ခုကို နောက်တစ်ချက် ပို့နိုင်ပါသည်။'
            : 'Send one screenshot or file next if it helps.',
        ],
      ),
    ],
    footerLines: [
      isMyanmar
        ? 'စီမံသူ အဖြေကို ဒီ chat ထဲမှာပဲ ရပါမည်။'
        : 'Admin replies arrive in this chat.',
    ],
  });
}
...
export function buildTelegramSupportTriageKeyboard(input: { locale: SupportedLocale }) {
  ...
  const rows = [
    [
      {
        text: isMyanmar ? '💳 ငွေပေးချေမှု' : '💳 Payment',
        callback_data: buildTelegramSupportThreadCallbackData('new', 'order'),
      },
      {
        text: isMyanmar ? '🔑 သော့' : '🔑 Key',
        callback_data: buildTelegramSupportThreadCallbackData('new', 'key'),
      },
    ],
    [
      {
        text: isMyanmar ? '🖥 ဆာဗာ' : '🖥 Server',
        callback_data: buildTelegramSupportThreadCallbackData('new', 'server'),
      },
      {
        text: isMyanmar ? '❓ အခြား' : '❓ Other',
        callback_data: buildTelegramSupportThreadCallbackData('new', 'general'),
      },
    ],
    [
